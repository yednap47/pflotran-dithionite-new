...
myvar = ["east CrO4-- [mol/d]","east Fe++ [mol/d]"]

redo = Pflotran.readObsDataset(filename * "-mas.dat",myvar;dataframe=false)
gold = Pflotran.readObsDataset(filename * "-norxn-mas.dat",myvar;dataframe=false)

plt.subplot(2,1,1)
plt.plot(redo[:,1],-redo[:,2],"-")
plt.plot(gold[:,1],-gold[:,2],"--")
plt.xlim(0,365)

plt.subplot(2,1,2)
plt.plot(redo[:,1],-redo[:,3],"-",label = "rxn")
plt.plot(gold[:,1],-gold[:,3],"--",label = "no rxn")
plt.xlim(0,365)
...
plt.ylabel(myvar[2])
plt.tight_layout()
plt.savefig("$(filename)-breakthrough.png",dpi=600)
plt.close()
