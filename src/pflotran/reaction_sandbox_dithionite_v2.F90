...
! 1. Change all references to "Dithionite_v2" as desired to rename the module and
...
! PetscReal, parameter :: formula_weight_of_water = 18.01534d0
  ...
  type, public, &
    extends(reaction_sandbox_base_type) :: reaction_sandbox_dithionite_v2_type
! 3. Add variables/arrays associated with new reaction.
  ...
  contains
    ...
    procedure, public :: Evaluate => Dithionite_v2React
    procedure, public :: UpdateKineticState => Dithionite_v2KineticState
    procedure, public :: Destroy => Dithionite_v2Destroy
  end type reaction_sandbox_dithionite_v2_type

  public :: Dithionite_v2Create

contains

! ************************************************************************** !

function Dithionite_v2Create()
  !
  ! Allocates dithionite_v2 reaction object.
  !
  ...
  class(reaction_sandbox_dithionite_v2_type), pointer :: Dithionite_v2Create

! 4. Add code to allocate object and initialized all variables to zero and
!    nullify all pointers. E.g.
  allocate(Dithionite_v2Create)
  ! Names
  Dithionite_v2Create%name_spec_h = '' ! H+
  Dithionite_v2Create%name_spec_o2 = '' ! O2(aq)
  Dithionite_v2Create%name_spec_cr6 = '' ! CrO4-
  Dithionite_v2Create%name_spec_cr3 = '' ! Cr+++
  Dithionite_v2Create%name_spec_s2o4 = '' ! S2O4--
  Dithionite_v2Create%name_spec_s2o3  = '' ! S2O3--
  ...
  ! IDs
  Dithionite_v2Create%id_spec_h = 0 ! H+
  Dithionite_v2Create%id_spec_o2 = 0 ! O2(aq)
  Dithionite_v2Create%id_spec_cr6 = 0 ! CrO4-
  Dithionite_v2Create%id_spec_cr3 = 0 ! Cr+++
  Dithionite_v2Create%id_spec_s2o4 = 0 ! S2O4--
  Dithionite_v2Create%id_spec_s2o3  = 0 ! S2O3--
  ...
  ! Rate constants
  Dithionite_v2Create%k_s2o4_disp = 0.d0
  Dithionite_v2Create%k_s2o4_o2 = 0.d0
  Dithionite_v2Create%k_s2o4_fe3 = 0.d0
  Dithionite_v2Create%k_fe2_o2_fast = 0.d0
  ...
  nullify(Dithionite_v2Create%next)

end function Dithionite_v2Create

! ************************************************************************** !

subroutine Dithionite_v2Read(this,input,option)
  !
  ! Reads input deck for dithionite_v2 reaction parameters (if any)
  !
  ! Author: Sachin Pandey
  ! Date: 09/08/16
  !
...
#include "petsc/finclude/petscsys.h"
  ...
  class(reaction_sandbox_dithionite_v2_type) :: this
    ...
    select case(trim(word))
      ...
      case default
        call InputKeywordUnrecognized(word, &
                     'CHEMISTRY,REACTION_SANDBOX,DITHIONITE_V2_PARAMETERS',option)
    end select
  enddo

end subroutine Dithionite_v2Read

! ************************************************************************** !

subroutine Dithionite_v2Setup(this,reaction,option)
  !
  ! Sets up the dithionite_v2 reaction either with parameters either
...
end subroutine Dithionite_v2Setup

! ************************************************************************** !

subroutine Dithionite_v2React(this,Residual,Jacobian,compute_derivative, &
...
! 11. If using an analytical Jacobian, add code for Jacobian evaluation
     ...
     option%io_buffer = 'NUMERICAL_JACOBIAN_RXN must always be used ' // &
                        'due to assumptions in Dithionite_v2'
     call printErrMsg(option)

  endif

end subroutine Dithionite_v2React

! ************************************************************************** !

subroutine Dithionite_v2KineticState(this,rt_auxvar,global_auxvar, &
...
end subroutine Dithionite_v2KineticState

! ************************************************************************** !

subroutine Dithionite_v2Destroy(this)
  !
...
! 12. Add code to deallocate contents of the dithionite_v2 object

end subroutine Dithionite_v2Destroy

end module Reaction_Sandbox_Dithionite_v2_class
